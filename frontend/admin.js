...
// --- TOAST BİLDİRİM FONKSİYONU (Admin) ---
window.showToast = (mesaj, tip = 'info', onClickCallback = null) => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnSilOnay').addEventListener('click', async () => {
        if (silinecekRandevuId) {
            try {
                const response = await fetch(`/api/randevular/${silinecekRandevuId}`, { method: 'DELETE' });
                if (response.ok) {
                    randevulariGetir();
                    silmeModal.style.display = 'none';
                    showToast('Randevu başarıyla silindi.', 'success');
                }
            } catch (hata) { showToast('Silme işlemi başarısız.', 'error'); }
        }
    });
...
async function yeniRandevuKontrol() {
    try {
        ...
        if (sonRandevuSayisi !== -1 && guncelSayi > sonRandevuSayisi) {
            ...
            const eskiIdler = new Set(tumRandevular.map(r => r._id || r.id));
            const yeniRandevular = guncelVeri.filter(r => !eskiIdler.has(r._id || r.id));
            ...
            showToast(`🔔 Yeni randevu: ${yeniRandevu.adSoyad}`, 'success', () => {
                detayGoster(yeniRandevu._id || yeniRandevu.id);
                });
            } else {
...
function tabloyuYenile() {
    ...
    randevular.forEach(randevu => {
        ...
        satir.onclick = (e) => {
            ...
            detayGoster(randevu._id || randevu.id);
        };

        const saatBilgisi = randevu.randevuSaati ? `<br><small>🕒 ${randevu.randevuSaati}</small>` : '';

        const durumDropdown = `
            <select class="status-dropdown" onchange="durumDropdownGuncelle('${randevu._id || randevu.id}', this.value)">
        ...
        satir.innerHTML = `
            ...
            <td>${durumDropdown}</td>
            <td>
                <button class="btn-action" style="background-color: #17a2b8; color: white;" onclick="detayGoster('${randevu._id || randevu.id}')" title="Detaylar">ℹ️</button>
                <button class="btn-action delete" onclick="randevuSil('${randevu._id || randevu.id}')">🗑️</button>
            </td>
        `;
...
window.durumDropdownGuncelle = (id, yeniDurum) => {
    const randevu = tumRandevular.find(r => String(r._id || r.id) === String(id));
...
async function durumGuncelle(id, yeniDurum, saat = null) {
    ...
    } catch (hata) { showToast('Güncelleme başarısız.', 'error'); }
}
